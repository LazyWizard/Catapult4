...
func load_available_fonts() -> void:
	...
	var json_result := test_json_conv.parse(f.get_as_text())
	...
	if json_result:
		...
		Status.post(tr("msg_font_info_error_details")
				% [json_result, test_json_conv.error_line, test_json_conv.error_string], Enums.MSG_DEBUG)
		return
		
	available_fonts = test_json_conv.data
...
func load_font_config() -> void:
	...
	if FileAccess.file_exists(config_file):
		...
		if f != null:
			var test_json_conv = JSON.new()
			var parse_result := test_json_conv.parse(f.get_as_text())
			...
			if parse_result == OK:
				...
				Status.post(tr("msg_font_config_error_details")
						% [parse_result, test_json_conv.error_line, test_json_conv.error_string], Enums.MSG_DEBUG)
		else:
...
func load_game_options() -> void:
	...
	if FileAccess.file_exists(options_file):
		...
		if f != null:
			...
			#var parse_result := test_json_conv.get_data()
			if parse_result == OK:
				_game_options = test_json_conv.data
			else:
